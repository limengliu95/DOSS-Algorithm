...
def readFromFolders(subset):
    ...
    #readResults("./results/RBFOpt_Plain/" + subset, set_prob, set_strategy, dict_probstrgy, result)
    readResults("./results/SRBF/" + subset, set_prob, set_strategy, dict_probstrgy, result)
...
# fig = plt.figure()
plt.subplots_adjust(top=0.977, bottom=0.065, left=0.124, right=0.95, hspace=0.2, wspace=0.24)
...
for strgy in set_strategy:
    ...
    plt.xlabel('Performance Ratio', fontdict={'weight': 'normal', 'size': 8})
    plt.ylabel('Fraction of Instances', fontdict={'weight': 'normal', 'size': 8})

    plt.tick_params(labelsize=8)
    # plt.title('Performance Profile on '+str(nprob)+' Problems (tau='+str(tau)+')', fontdict={'weight': 'normal', 'size': 8})
    plt.legend(fontsize=8)
...
for strgy in set_strategy:
    ...
    plt.xlabel('Number of Equivalent Gradient Iterations', fontdict={'weight': 'normal', 'size': 8})
    plt.ylabel('Fraction of Instances', fontdict={'weight': 'normal', 'size': 8})

    plt.tick_params(labelsize=8)
    # plt.title('Data Profile on '+str(nprob)+' Problems (tau='+str(tau)+')', fontdict={'weight': 'normal', 'size': 8})
    plt.legend(fontsize=8)

    strgyid += 1

# plt.grid()
plt.savefig('./figure/BBOBprofile5e-3_1.pdf')
